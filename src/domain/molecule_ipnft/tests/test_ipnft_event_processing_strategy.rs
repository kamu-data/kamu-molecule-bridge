...
async fn test_initial_owner_transfers_to_multisig_they_participate_in() {
    ...
    assert_eq!(
        [
            ...
            IpnftEventProcessingDecision::AddMultisigToIndexing {
                ipnft_uid,
                address: multisig,
            },
        ],
...
#[tokio::test]
async fn test_no_action_if_ipnft_minted_and_immediately_burnt() {
    let ipnft_uid = IpnftUid {
        ipnft_address: address!("0x1010101010101010101010101010101010101010"),
        token_id: U256::from(1),
    };
    ...
    let events = vec![
        ...
        IpnftEvent::Burnt(IpnftEventBurnt {
            ipnft_uid,
            former_owner: multisig,
        }),
    ];
    ...
    assert_eq!([] as [IpnftEventProcessingDecision; 0], *decisions);
}
